...
class Seq2Seq(object):
	def __init__(self, xseq_len, yseq_len,
		...
		def __graph__():
			...
			# stack cells together
			stacked_lstm = tf.contrib.rnn.core_rnn_cell.MultiRNNCell([basic_cell]*num_layers	,
																	 state_is_tuple=True)
			# For parameter sharing between training models
			# and testing model
			with tf.variable_scope('decoder') as scope:
				...
				self.decode_outputs, self.decode_states = tf.contrib.legacy_seq2seq.embedding_rnn_seq2seq(self.enc_ip,
														self.dec_ip, stacked_lstm, xvocab_size, yvocab_size, emb_dim)
				# share parameters
				scope.reuse_variables()
				# testing model, where output of previous timestamp is fed as input
	...
	def eval_step(self, sess, eval_batch_gen):
		...
		# dec_op_v is a list; also need to transpose 0,1 indicecs
		#  (interchange batch_size and timesteps dimensions)
		dec_op_v, np.array(dec_op_v).transpose([1,0,2])
		return loss_v, dec_op_v, batchX, batchY
	# evaluate 'num_batches' batches
	def eval_bacthes(self, sess, eval_batch_gen, num_batches):
		losses = []
	...
	# evaluate on valid set periodically
	# prints statstics
		...
		for i in range(self.epochs):
			try:
				...
				if i and i%(self.epochs//100) == 0 :
					...
					saver.save(sess, self.ckpt_path + self.model_name + '.ckpt_path', global_step=i)
					# evaluate to get the validation loss
					val_loss = self.eval_bacthes(sess, valid_set, 16)
					# print stats
					print('\nModel saved to the disk at iteration #{}'.format(i))
					print('val loss : {0:.6f}'.format(val_loss))
					sys.stdout.flush()
			except KeyboardInterrupt: # this will most definitely happen, so handle it
				print("Iterrputed by the user at iteration #{}".format(i))
				self.session = sess
				return sess
